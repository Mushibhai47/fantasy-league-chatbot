...
class PlayerMatcher:
    ...
    def __init__(self, db: Session):
        ...
        self._sleeper_id_index: Dict[str, Player] = {}
    ...
    def _ensure_cache(self):
        ...
        self._sleeper_id_index = {
            p.sleeper_id: p for p in self._all_players if p.sleeper_id
        }
    ...
    def _add_to_cache(self, player: Player):
        ...
        if player.sleeper_id:
            self._sleeper_id_index[player.sleeper_id] = player
    ...
    def match_player(self, player_data: Dict) -> Optional[Player]:
        ...
        if player_data.get('sleeper_id'):
            self._ensure_cache()
            match = self._sleeper_id_index.get(str(player_data['sleeper_id']))
            if match:
                return match
    ...
    def get_or_create_player(self, player_data: Dict) -> Player:
        ...
        if player:
            ...
            if player_data.get('yahoo_id') and not player.yahoo_id:
                ...
                self._yahoo_id_index[player.yahoo_id] = player
            if player_data.get('sleeper_id') and not player.sleeper_id:
                player.sleeper_id = str(player_data['sleeper_id'])
                self._sleeper_id_index[player.sleeper_id] = player
